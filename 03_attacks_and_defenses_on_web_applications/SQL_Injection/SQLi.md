...
![alt text](image-7.png) That worked, so no we can query to version to pull oracle sensitive data.
...
### Sanitization
Cleaning a string to escape, by adding backslashes. Altough it is error prone, we can isntead use ORM's (Object Relational Mapper) like Hibernate, Entity Framework, Eloquent, etc.